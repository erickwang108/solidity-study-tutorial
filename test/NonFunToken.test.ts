...
describe("NonFunToken", function () {
  ...
  beforeEach(async function () {
    ...
    this.contractOwner = owner.address;
    this.collector = signer1.address;

    // Get the collector contract fof signing transaction with collector key
    this.collectorContract = this.nonFunToken.connect(signer1);

    // Mint an initial set of NFTs from this collection
    this.initialMintCount = 3;
    this.initialMint = [];
    for (let k = 1; k < this.initialMintCount; k += 1) {
      await this.nonFunToken.mintCollectionNFT(this.contractOwner, k);
      this.initialMint.push(k.toString());
    }
  });
  ...
  it("Mints initial set of NFTs from collection to contractOwner", async function () {
    for (let i = 0; i < this.initialMint.length; i++) {
      expect(await this.nonFunToken.ownerOf(this.initialMint[i])).to.equal(
        this.contractOwner
      );
    }
  });
  ...
  it("Is able to query the NFT balances of an address", async function () {
    expect(await this.nonFunToken.balanceOf(this.contractOwner)).to.equal(
      this.initialMint.length
    );
  });
  ...
  it("Emits a transfer event for newly minted NFTs", async function () {
    const tokenId = (this.initialMint.length + 1).toString();
    await expect(
      this.nonFunToken.mintCollectionNFT(this.contractOwner, tokenId)
    )
      .to.emit(this.nonFunToken, "Transfer")
      .withArgs(ZERO_ADDRESS, this.contractOwner, tokenId); // NFTs are minted from zero address
  });
  ...
  it("Is able to transfer NFTs to another wallet when called by owner", async function () {
    ...
    await this.nonFunToken["safeTransferFrom(address,address,uint256)"](
      this.contractOwner,
      this.collector,
      tokenId
    );
  ...
  it("Emits a Transfer event when transferring a NFT", async function () {
    const tokenId = this.initialMint[0].toString();
    await expect(
      this.nonFunToken["safeTransferFrom(address,address,uint256)"](
        this.contractOwner,
        this.collector,
        tokenId
      )
  ...
  it("Approves an operator wallet to spend owner NFT", async function () {
    ...
    expect(await this.nonFunToken.getApproved(tokenId)).to.equal(
      this.collector
    );
  });
  ...
  it("Allows operator to transfer NFT on behalf of owner", async function () {
    ...
    await this.collectorContract["safeTransferFrom(address,address,uint256)"](
      this.contractOwner,
      this.collector,
      tokenId
    );
  ...
  it("Approves an operator to spend all of an owner's NFTs", async function () {
    ...
    expect(
      await this.nonFunToken.isApprovedForAll(
        this.contractOwner,
        this.collector
      )
    ).to.equal(true);
  ...
  it("Allows operator to transfer all NFTs on behalf of owner", async function () {
    ...
    expect(await this.nonFunToken.balanceOf(this.collector)).to.equal(
  ...
  it("Only allows contractOwner to mint NFTs", async function () {
    await expect(
      this.collectorContract.mintCollectionNFT(this.collector, "100")
    ).to.be.reverted;
  });
});
